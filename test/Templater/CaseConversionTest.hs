...
test_detectValueStyle = fmap runTest cases ++ fmap runNegativeTest negativeCases
 where
  runTest (str, tw)  =
  ...
  runNegativeTest (str, tw)  =
      let values = textToTemplateValues str
      in  testCase (T.unpack str <> " is not a " <> show tw) (tw `assertNotElem` values)
  cases =
    [ ( "aaaBbbCcc"
...
scprop_multiWordTemplateValueLength testText =
  let text   = T.pack $ show testText
      ...
      valid (MultiWordTemplateValue fw ow ss tt) = if length tt >= 2
        then Right ()
        else Left (show (MultiWordTemplateValue fw ow ss tt) <> " has only " <> show (length tt) <> " word")
      valid _ = Right ()
